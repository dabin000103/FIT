...
namespace AirWebService
{
	...
	public class SabreAirService : System.Web.Services.WebService
	{
        ...
        #region "결제(카드/TASF/현금영수증)"

        #region "신용카드 승인"

        public XmlElement CardApprovalRQ(string TxnType, string CardNo, string CardExt, string CardType, int InstMonth, string AirCode, int Amount)
        {
            XmlDocument XmlDoc = new XmlDocument();
            XmlDoc.Load(sc.XmlFullPath("CardApprovalRQ"));

            XmlNamespaceManager xnMgr = new XmlNamespaceManager(XmlDoc.NameTable);
            xnMgr.AddNamespace("pay", SabreConfig.NamespaceURL("CardApproval"));

            XmlNode CardApprovalRequest = XmlDoc.SelectSingleNode("pay:CardApproval/CardApprovalRequest", xnMgr);

            CardApprovalRequest.SelectSingleNode("TYPE").InnerText = "A";
            CardApprovalRequest.SelectSingleNode("TXN_TYPE").InnerText = TxnType;
            CardApprovalRequest.SelectSingleNode("CARD_NO").InnerText = CardNo.Replace("-", "");
            CardApprovalRequest.SelectSingleNode("CARD_EXT").InnerText = CardExt;
            CardApprovalRequest.SelectSingleNode("INST_MONTH").InnerText = cm.NumPosition(String.Concat("00", InstMonth), 2);
            CardApprovalRequest.SelectSingleNode("CARD_TYPE").InnerText = CardType;
            CardApprovalRequest.SelectSingleNode("AIRLINE_2CD").InnerText = AirCode;
            CardApprovalRequest.SelectSingleNode("AMOUNT").InnerText = Amount.ToString();
            CardApprovalRequest.SelectSingleNode("DEVICE_ID").InnerText = sc.UserName;
            CardApprovalRequest.SelectSingleNode("PCC").InnerText = sc.PCC;
            CardApprovalRequest.SelectSingleNode("AUTH_ID").InnerText = sc.AUTHID;
            CardApprovalRequest.SelectSingleNode("AUTH_PW").InnerText = sc.AUTHPW;
            CardApprovalRequest.RemoveChild(CardApprovalRequest.SelectSingleNode("APPR_NO"));
        ...
        /// <param name="CardExt">유효기간(MMYY)</param>
        /// <param name="CardType">카드종류</param>
        /// <param name="InstMonth">할부개월수(0:일시불)</param>
        /// <param name="AirCode">항공사코드</param>
        /// <param name="Amount">결제금액</param>
        /// <param name="GUID">고유번호</param>
        /// <returns></returns>
        [WebMethod(Description = "신용카드 승인")]
        public XmlElement CardApprovalRS(string CardNo, string CardExt, string CardType, int InstMonth, string AirCode, int Amount, string GUID)
        {
            XmlElement ReqXml = CardApprovalRQ("TK", CardNo, CardExt, CardType, InstMonth, AirCode, Amount);
            cm.XmlFileSave(ReqXml, sc.Name, "CardApprovalRQ", "N", GUID);

            XmlElement ResXml = sc.HttpExecuteSoapHeader("CardApproval", ReqXml, GUID);
            cm.XmlFileSave(ResXml, sc.Name, "CardApprovalRS", "N", GUID);
        ...
        public XmlElement CardApprovalCancelRQ(string TxnType, string CardNo, string CardExt, string CardType, int InstMonth, string AirCode, int Amount, int ApprNo)
        {
            XmlDocument XmlDoc = new XmlDocument();
            XmlDoc.Load(sc.XmlFullPath("CardApprovalRQ"));
            ...
            CardApprovalRequest.SelectSingleNode("TYPE").InnerText = "C";
            CardApprovalRequest.SelectSingleNode("TXN_TYPE").InnerText = TxnType;
            CardApprovalRequest.SelectSingleNode("CARD_NO").InnerText = CardNo.Replace("-", "");
            CardApprovalRequest.SelectSingleNode("CARD_EXT").InnerText = CardExt;
            CardApprovalRequest.SelectSingleNode("INST_MONTH").InnerText = cm.NumPosition(String.Concat("00", InstMonth), 2);
            ...
            CardApprovalRequest.SelectSingleNode("AUTH_ID").InnerText = sc.AUTHID;
            CardApprovalRequest.SelectSingleNode("AUTH_PW").InnerText = sc.AUTHPW;
            CardApprovalRequest.SelectSingleNode("APPR_NO").InnerText = ApprNo.ToString();
        ...
        /// <param name="AirCode">항공사코드</param>
        /// <param name="Amount">결제금액</param>
        /// <param name="ApprNo">승인번호</param>
        /// <param name="GUID">고유번호</param>
        /// <returns></returns>
        [WebMethod(Description = "신용카드 승인 취소")]
        public XmlElement CardApprovalCancelRS(string CardNo, string CardExt, string CardType, int InstMonth, string AirCode, int Amount, int ApprNo, string GUID)
        {
            XmlElement ReqXml = CardApprovalCancelRQ("TK", CardNo, CardExt, CardType, InstMonth, AirCode, Amount, ApprNo);
            cm.XmlFileSave(ReqXml, sc.Name, "CardApprovalCancelRQ", "N", GUID);

            XmlElement ResXml = sc.Execute("CardApproval", ReqXml);
            cm.XmlFileSave(ResXml, sc.Name, "CardApprovalCancelRS", "N", GUID);

            return ResXml;
        }

        #endregion "신용카드 승인 취소"
        ...
        /// <param name="GUID">고유번호</param>
        /// <returns></returns>
        [WebMethod(Description = "TASF 승인")]
        public XmlElement TasfApprovalRS(string CardNo, string CardExt, string CardType, int InstMonth, string AirCode, int Amount, string GUID)
        {
            XmlElement ReqXml = CardApprovalRQ("TA", CardNo, CardExt, CardType, InstMonth, AirCode, Amount);
            cm.XmlFileSave(ReqXml, sc.Name, "TasfApprovalRQ", "N", GUID);

            XmlElement ResXml = sc.Execute("CardApproval", ReqXml);
            cm.XmlFileSave(ResXml, sc.Name, "TasfApprovalRS", "N", GUID);
        ...
        /// <param name="GUID">고유번호</param>
        /// <returns></returns>
        [WebMethod(Description = "TASF 승인 취소")]
        public XmlElement TasfApprovalCancelRS(string CardNo, string CardExt, string CardType, int InstMonth, string AirCode, int Amount, int ApprNo, string GUID)
        {
            XmlElement ReqXml = CardApprovalCancelRQ("TA", CardNo, CardExt, CardType, InstMonth, AirCode, Amount, ApprNo);
            cm.XmlFileSave(ReqXml, sc.Name, "TasfApprovalCancelRQ", "N", GUID);

            XmlElement ResXml = sc.Execute("CardApproval", ReqXml);
            cm.XmlFileSave(ResXml, sc.Name, "TasfApprovalCancelRS", "N", GUID);

            return ResXml;
        }

        #endregion "TASF 승인 취소"

        #region "현금영수증 승인"

        public XmlElement CashReceiptRQ(string AirCode, int Amount, string TktNo, string IdentityNo, string CorpType)
        {
            XmlDocument XmlDoc = new XmlDocument();
            XmlDoc.Load(sc.XmlFullPath("CashReceiptRQ"));
            ...
            CardApprovalRequest.SelectSingleNode("TYPE").InnerText = "A";
            CardApprovalRequest.SelectSingleNode("TXN_TYPE").InnerText = "TR";
            CardApprovalRequest.SelectSingleNode("AIRLINE_2CD").InnerText = AirCode;
            CardApprovalRequest.SelectSingleNode("AMOUNT").InnerText = Amount.ToString();
            CardApprovalRequest.SelectSingleNode("DEVICE_ID").InnerText = sc.UserName;
            CardApprovalRequest.SelectSingleNode("PCC").InnerText = sc.PCC;
            CardApprovalRequest.SelectSingleNode("AUTH_ID").InnerText = sc.AUTHID;
            CardApprovalRequest.SelectSingleNode("AUTH_PW").InnerText = sc.AUTHPW;
            CardApprovalRequest.SelectSingleNode("TKT_NO").InnerText = TktNo;
            CardApprovalRequest.SelectSingleNode("IDENTITY_NO").InnerText = IdentityNo.Replace("-", "");
            CardApprovalRequest.SelectSingleNode("CORP_TYPE").InnerText = CorpType;
            CardApprovalRequest.SelectSingleNode("SOURCE_IND").InnerText = "I";

            CardApprovalRequest.RemoveChild(CardApprovalRequest.SelectSingleNode("APPR_NO"));
            CardApprovalRequest.RemoveChild(CardApprovalRequest.SelectSingleNode("REASON_CD"));
        ...
        /// <param name="IdentityNo">식별번호(휴대폰번호)</param>
        /// <param name="CorpType">발행구분(01:개인, 02:법인)</param>
        /// <param name="GUID">고유번호</param>
        /// <returns></returns>
        [WebMethod(Description = "현금영수증 승인")]
        public XmlElement CashReceiptRS(string AirCode, int Amount, string TktNo, string IdentityNo, string CorpType, string GUID)
        {
            XmlElement ReqXml = CashReceiptRQ(AirCode, Amount, TktNo, IdentityNo, CorpType);
            cm.XmlFileSave(ReqXml, sc.Name, "CashReceiptRQ", "N", GUID);

            XmlElement ResXml = sc.Execute("CardApproval", ReqXml);
            cm.XmlFileSave(ResXml, sc.Name, "CashReceiptRS", "N", GUID);
        ...
        public XmlElement CashReceiptCancelRQ(string AirCode, int Amount, int ApprNo, string TktNo, string IdentityNo, string CorpType, string ReasonCd)
        {
            XmlDocument XmlDoc = new XmlDocument();
            XmlDoc.Load(sc.XmlFullPath("CashReceiptRQ"));
            ...
            CardApprovalRequest.SelectSingleNode("CORP_TYPE").InnerText = CorpType;
            CardApprovalRequest.SelectSingleNode("SOURCE_IND").InnerText = "I";
            CardApprovalRequest.SelectSingleNode("REASON_CD").InnerText = ReasonCd;
        ...
        /// <param name="IdentityNo">식별번호(휴대폰번호)</param>
        /// <param name="CorpType">발행구분(01:개인, 02:법인)</param>
        /// <param name="ReasonCd">취소사유(01:거래취소, 02:오류발급, 03:기타취소)</param>
        /// <param name="GUID">고유번호</param>
        /// <returns></returns>
        [WebMethod(Description = "현금영수증 승인 취소")]
        public XmlElement CashReceiptCancelRS(string AirCode, int Amount, int ApprNo, string TktNo, string IdentityNo, string CorpType, string ReasonCd, string GUID)
        {
            XmlElement ReqXml = CashReceiptCancelRQ(AirCode, Amount, ApprNo, TktNo, IdentityNo, CorpType, ReasonCd);
            cm.XmlFileSave(ReqXml, sc.Name, "CashReceiptCancelRQ", "N", GUID);

            XmlElement ResXml = sc.Execute("CardApproval", ReqXml);
            cm.XmlFileSave(ResXml, sc.Name, "CashReceiptCancelRS", "N", GUID);

            return ResXml;
        }

        #endregion "현금영수증 승인 취소"

        #endregion "결제(카드/TASF/현금영수증)"
