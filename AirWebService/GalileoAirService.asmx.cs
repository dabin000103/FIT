...
namespace AirWebService
{
    ...
    public class GalileoAirService : System.Web.Services.WebService
    {
        ...
        public string TicketIssuingRQ(string ReservationCode, string PNR, string GoodCode, string ReqID, int PaxCnt, string PaxAmountInfo, string KeyInOK, string FQMethod, string GDSType, string PNR2, string GDSType2)
        {
            return String.Format("ReservationCode={0}&PNR={1}&GoodCode={2}&ReqID={3}&PaxCnt={4}&PaxAmountInfo={5}&KeyInOK={6}&FQMethod={7}&GDSType={8}&PNR2={9}&GDSType2={10}&CustomerTkt=R&enc=u", ReservationCode, PNR, GoodCode, ReqID, PaxCnt, PaxAmountInfo, KeyInOK, FQMethod, GDSType, PNR2, GDSType2);
        }
        ...
        /// <param name="KeyInOK">KE 키인승인 허용여부(키인으로 승인 받는 경우만 Y로 전달)</param>
        /// <param name="FQMethod">쿼트/발권 구분(FQ, TK)</param>
        /// <param name="GDSType">GDS 구분(G:Galileo(1G), A/D:Amadeus(1A), B:Sabre(1B))</param>
        /// <param name="PNR2">GK 발권 처리용 PNR(세이버의 GK PNR)</param>
        /// <param name="GDSType2">GK 발권 처리용 PNR 의 GDSType(GK PNR일 경우 B 값으로 설정)</param>
        /// <param name="GUID">고유번호</param>
        /// <returns></returns>
        [WebMethod(Description = "자동발권")]
        public XmlElement TicketIssuingRS(string ReservationCode, string PNR, string GoodCode, string ReqID, int PaxCnt, string PaxAmountInfo, string KeyInOK, string FQMethod, string GDSType, string PNR2, string GDSType2, string GUID)
        {
            string Parameters = TicketIssuingRQ(ReservationCode, PNR, GoodCode, ReqID, PaxCnt, PaxAmountInfo, KeyInOK, FQMethod, GDSType, PNR2, GDSType2);
