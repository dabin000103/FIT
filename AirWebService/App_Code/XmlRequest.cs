...
namespace AirWebService
{
	...
	public class XmlRequest
	{
        ...
        /// <summary>
        /// Soap Form 생성(세이버 카드결제용)
        /// </summary>
        /// <param name="XmlData">XML 데이타</param>
        /// <returns></returns>
        public static string SoapHeaderForSabreCardApproval(string XmlData)
        {
            return String.Format("<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:pay=\"http://payment.ws.fep.abacus.com/\"><soapenv:Header/><soapenv:Body>{0}</soapenv:Body></soapenv:Envelope>", XmlData);
        }
        ...
        public static XmlElement AmadeusSoapSend(string Url, string Action, string ServiceName, string XmlData, string GUID)
		{
            ...
            cm.XmlFileSave(ReqXml, "AmadeusSoapSend", String.Format("{0}RQ-Soap", ServiceName), "N", GUID);
            ...
            try
            {
                ...
                using (HttpWebResponse res = (HttpWebResponse)req.GetResponse())
                {
                    using (StreamReader resStream = new StreamReader(res.GetResponseStream(), Encoding.GetEncoding("UTF-8")))
                    {
                        ...
                        //ResXml.LoadXml(cm.StringFileSave(resStream.ReadToEnd(), "AmadeusSoapSend", "SendToXml_Success", GUID));
                    }
                }
            ...
            catch (WebException ex)
            {
                using (WebResponse res = ex.Response)
                {
                    ...
                    using (StreamReader resStream = new StreamReader(res.GetResponseStream(), Encoding.UTF8))
                    {
                        ...
                        //XmlDoc.LoadXml(cm.StringFileSave(resStream.ReadToEnd(), "AmadeusSoapSend", "SendToXml_Error", GUID));
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                //XmlDoc.LoadXml(String.Format("<result><statusInfo><status>FAILE</status><returnMessage><![CDATA[{0}]]></returnMessage><datetime>{1}</datetime></statusInfo></result>", cm.StringFileSave(ex.ToString(), "AmadeusSoapSend", "SendToXml_Exception", GUID), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")));
            }
        ...
        public static XmlElement AmadeusSoapSendHeader4(string Url, string Action, string ServiceName, string XmlData, string GUID)
        {
            ...
            XmlDocument ReqXml = new XmlDocument();
            ReqXml.LoadXml(XmlData);
            cm.XmlFileSave(ReqXml, "AmadeusSoapSendHeader4", String.Format("{0}RQ-Soap", ServiceName), "N", GUID);
            ...
            try
            {
                ...
                using (HttpWebResponse res = (HttpWebResponse)req.GetResponse())
                {
                    using (StreamReader resStream = new StreamReader(res.GetResponseStream(), Encoding.GetEncoding("UTF-8")))
                    {
                        ...
                        //Push Data 저장
                        //ResXml.LoadXml(cm.StringFileSave(resStream.ReadToEnd(), "AmadeusSoapSendHeader4", String.Format("{0}_Success", Action), GUID));
                    }
                }
            ...
            catch (WebException ex)
            {
                using (WebResponse res = ex.Response)
                {
                    ...
                    using (StreamReader resStream = new StreamReader(res.GetResponseStream(), Encoding.UTF8))
                    {
                        ...
                        //Push Data 저장
                        XmlDoc.LoadXml(cm.StringFileSave(resStream.ReadToEnd(), "AmadeusSoapSendHeader4", String.Format("{0}_Error", Action), GUID));
                    }
                }
            }
            catch (Exception ex)
            {
                //XmlDoc.LoadXml(String.Format("<result><statusInfo><status>FAILE</status><returnMessage><![CDATA[{0}]]></returnMessage><datetime>{1}</datetime></statusInfo></result>", ex.ToString(), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")));
                XmlDoc.LoadXml(String.Format("<result><statusInfo><status>FAILE</status><returnMessage><![CDATA[{0}]]></returnMessage><datetime>{1}</datetime></statusInfo></result>", cm.StringFileSave(ex.ToString(), "AmadeusSoapSendHeader4", String.Format("{0}_Exception", Action), GUID), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")));
            }
        ...
        /// <summary>
        /// SOAP 방식을 이용하여 데이타 전송 후 결과값 리턴(세이버 카드결제용)
        /// </summary>
        /// <param name="Url">End Point</param>
        /// <param name="Action">Action URL</param>
        /// <param name="ServiceName">서비스명</param>
        ...
        /// <param name="GUID">고유번호</param>
        /// <returns></returns>
        public static XmlElement SabreSoapSendCardApproval(string Url, string Action, string ServiceName, string XmlData, string GUID)
        {
            Common cm = new Common();
            XmlDocument ReqXml = new XmlDocument();
            ReqXml.LoadXml(XmlData);
            cm.XmlFileSave(ReqXml, "SabreSoapSendCardApproval", String.Format("{0}RQ-Soap", ServiceName), "N", GUID);
            ...
            try
            {
                ...
                using (HttpWebResponse res = (HttpWebResponse)req.GetResponse())
                {
                    ...
                    {
                        //ResXml.LoadXml(resStream.ReadToEnd());
                        ResXml.LoadXml(cm.StringFileSave(resStream.ReadToEnd(), "SabreSoapSendCardApproval", "SendToXml_Success", GUID));
                    }
                }
            ...
            catch (WebException ex)
            {
                using (WebResponse res = ex.Response)
                {
                    ...
                    {
                        //XmlDoc.LoadXml(resStream.ReadToEnd());
                        XmlDoc.LoadXml(cm.StringFileSave(resStream.ReadToEnd(), "SabreSoapSendCardApproval", "SendToXml_Error", GUID));
                    }
                }
            }
            catch (Exception ex)
            {
                //XmlDoc.LoadXml(String.Format("<result><statusInfo><status>FAILE</status><returnMessage><![CDATA[{0}]]></returnMessage><datetime>{1}</datetime></statusInfo></result>", ex.ToString(), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")));
                XmlDoc.LoadXml(String.Format("<result><statusInfo><status>FAILE</status><returnMessage><![CDATA[{0}]]></returnMessage><datetime>{1}</datetime></statusInfo></result>", cm.StringFileSave(ex.ToString(), "SabreSoapSendCardApproval", "SendToXml_Exception", GUID), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")));
            }
        ...
        /// <param name="GUID">고유번호</param>
        /// <returns>XML 형식의 결과 Data</returns>
        public static string TopasSendToJson(string URL, string Method, string Data, string ItemGubun, string GUID)
        {
            Common cm = new Common();
            ...
            {
                //RQ저장
                cm.StringFileSave(String.Format("URL: {0}{3}METHOD: {1}{3}DATA: {2}", URL, Method, Data, Environment.NewLine), "Topas", "TopasSendToJson_SourceData", GUID);
                ...
                {
                    //Push Data 저장
                    ReturnJson = cm.StringFileSave(resStream.ReadToEnd(), "Topas", "TopasSendToJson_Success", GUID);
                }
            }
            ...
            //            {
            //                //Push Data 저장
            //                ReturnJson = cm.StringFileSave(resStream.ReadToEnd(), "Topas", "TopasSendToJson_Error", GUID);
            //            }
            //        }
            //    }
            //    catch (Exception ex)
            //    {
            //        ReturnJson = String.Format("{\"WSErrors\":{\"WSError\":{\"ExceptionInfo\":{\"ToString\":\"{0}\"}}}}", cm.StringFileSave(ex.ToString(), "Topas", "TopasSendToJson_WebException", GUID));
            //    }
            //}
            catch (Exception ex)
            {
                ReturnJson = String.Format("{\"WSErrors\":{\"WSError\":{\"ExceptionInfo\":{\"ToString\":\"{0}\"}}}}", cm.StringFileSave(ex.ToString(), "Topas", "TopasSendToJson_Exception", GUID));
            }
