...
{
	/// <summary>
	/// 웹서비스용 Exception 재정의(통합용)
	/// </summary>
	public class MWSExceptionMode : Exception
	{
        private const string ItemCode = "IA";
		...
		public XmlElement ToErrors
		{
			get {
                ...
                XmlErr.Load(new ModeConfig().XmlFullPath("Errors"));

                XmlErr.SelectSingleNode("ErrorMessage").Attributes.GetNamedItem("timeStamp").InnerText = new Common().TimeStamp;
				XmlErr.SelectSingleNode("ErrorMessage/errorSource/gds").InnerText = "Mode";
				XmlErr.SelectSingleNode("ErrorMessage/errorSource/method").InnerText = WebServiceName;
                XmlErr.SelectSingleNode("ErrorMessage/errorSource/service").InnerText = ServiceName;
        ...
        public MWSExceptionMode(Exception ex, HttpContext hcc, string GUID, string ServiceName, string WebServiceName, int ServiceNumber, int OrderNumber, int ItemBookingNumber)
			: base(ex.ToString())
		{
            ...
            DBSave(OrderNumber, ItemBookingNumber, GUID, ServiceName, WebServiceName, ServerAddress, WebServiceURL, UserHostAddress, UserAgent, HttpMethod, FormString, QueryString, Data, HelpLink, HResult, InnerException, Message, Source, StackTrace, TargetSite, Description, ServiceNumber);
            SendException(hcc, XmlDoc.DocumentElement, GUID, ServiceName, WebServiceName);
		...
		/// 예외에 대한 데이타 저장
		/// </summary>
        protected void SendException(HttpContext hcc, XmlElement XmlEx, string GUID, string ServiceName, string WebServiceName)
        {
            mXmlElmError = XmlEx;
            ...
            try
            {
                ...
                string FolderPath = String.Format(@"{0}WebServiceLog2\{1}\ErrorXml\{2}\{3}\{4}\", hcc.Request.PhysicalApplicationPath.Substring(0, 3), "AirWebService", "Mode", NowDate.ToString("yyyyMM"), NowDate.ToString("dd"));
                string FileName = String.Format("{0}_{1}({2}).xml", GUID, WebServiceName, Path.GetFileNameWithoutExtension(Path.GetRandomFileName()));
        ...
        protected void DBSave(int OrderNumber, int ItemBookingNumber, string GUID, string ServiceName, string WebServiceName, string ServerAddress, string WebServiceURL, string UserHostAddress, string UserAgent, string HttpMethod, string FormString, string QueryString, string Data, string HelpLink, string HResult, string InnerException, string Message, string Source, string StackTrace, string TargetSite, string Description, int ServiceNumber)
		{/*
			using (SqlCommand cmd = new SqlCommand())
			{
                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["WSVNSYNC"].ConnectionString))
				{
					...
					cmd.CommandText = "DBO.WSV_T_웹서비스_오류_통합_저장";
					...
					cmd.Parameters.Add("@WebServiceURL", SqlDbType.VarChar, 3000);
                    ...
                    cmd.Parameters.Add("@서비스번호", SqlDbType.Int, 0);
                    ...
                    cmd.Parameters["@품목코드"].Value = ItemCode;
                    ...
                    cmd.Parameters["@서비스번호"].Value = ServiceNumber;
